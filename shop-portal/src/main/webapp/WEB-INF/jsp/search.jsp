...
<!-- header end -->
<div class="w main">
	<div class="crumb">全部结果&nbsp;&gt;&nbsp;<strong>"${param.q}"</strong></div>
<div class="clr"></div>
<div class="m clearfix" id="bottom_pager">
<div  id="pagin-btm" class="pagin fr" clstag="search|keycount|search|pre-page2">
	<a href="search.html?q=${param.q}&page=${data.prev}" class="prev-disabled">
		上一页<b></b></a>
	
	<c:forEach begin="1" end="${data.totalPage }" var="n">
		<c:if test="${data.curr==n}">
			<a href="javascript:void(0)" class="current">${n}</a>
		</c:if>
		<c:if test="${data.curr!=n}">
			<a href="search.html?q=${param.q}&page=${n}">${n}</a>
		</c:if>
	</c:forEach>

		<!-- <a href="search?keyword=java&enc=utf-8&qr=&qrst=UNEXPAND&rt=1&page=6">雯</a>
		<span class="text">…</span> -->
	<a href="search.html?q=${param.q}&page=${data.next}" class="next">下一页<b></b></a>
	<span class="page-skip"><em>&nbsp;&nbsp;查询到${data.totalCount}条记录，共${data.totalPage}页</em></span>
</div>
</div>
...
SEARCH.bottom_page_html(${data.curr},${data.totalPage},'');
</script>
</body>
</html>